...
def build_summary(all_dfs):
    ...
    for filename, df in all_dfs.items():
        df = df[df["recipient_committee_type"].isin(
        ...
        grouped = (
            df.groupby(["_cycle", "recipient_committee_type", "_party"])
            ["disbursement_amount"].sum().reset_index()
        )
        cycle_totals = df.groupby("_cycle")["disbursement_amount"].sum().to_dict()
        ...
        for _, r in grouped.iterrows():
            ...
            cycle_total = cycle_totals.get(cycle, 0)
            pct         = round(100 * amount / cycle_total, 1) if cycle_total else 0
            ...
            rows.append({
                ...
                "Party":            party,
                "$ Amount":         amount,
                "% of Cycle Total": pct / 100,
                "Cycle Total ($)":  cycle_total,
            })
    ...
    with pd.ExcelWriter(summary_path, engine="openpyxl") as writer:
        ...
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
            for cell in row:
                ...
                if col_name in ("$ Amount", "Cycle Total ($)"):
                    cell.number_format = '$#,##0'
                elif col_name == "% of Cycle Total":
