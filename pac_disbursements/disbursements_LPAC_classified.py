...
OUTPUT_FOLDER     = os.path.join(ROOT_FOLDER, "disbursements_analyzed_wLPAC")
LPAC_EXPLODED     = os.path.join(ROOT_FOLDER, "fec_lpac_exploded.xlsx")
KEEP_ENTITY_TYPES = {"POLITICAL ACTION COMMITTEE", "CAMPAIGN COMMITTEE"}

# ── Diagnostics ───────────────────────────────────────────────────────────────
print(f"Script location:  {__file__}")
print(f"Root folder:      {ROOT_FOLDER}")
print(f"LPAC file:        {LPAC_EXPLODED}")
print(f"LPAC file exists: {os.path.exists(LPAC_EXPLODED)}")
...
def pick_input_files():
    try:
        ...
        paths = filedialog.askopenfilenames(
            title="Select one or more Schedule B disbursement CSVs",
...
# ── Process each CSV ──────────────────────────────────────────────────────────
csv_files = pick_input_files()
if not csv_files:
    raise FileNotFoundError("No files selected.")

print(f"Found {len(csv_files)} CSV file(s) selected\n")
print("=" * 60)
...
for i, csv_path in enumerate(csv_files, 1):
    ...
    try:
        ...
        if "entity_type_desc" not in df_raw.columns:
            print(f"  SKIP: missing 'entity_type_desc' column — not a disbursement file.\n")
            continue
...
ok = df_log[df_log["status"] == "OK"] if "status" in df_log.columns else pd.DataFrame()
if not ok.empty:
    print(f"\nAggregated totals:")
